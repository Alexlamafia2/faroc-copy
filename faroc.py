...
class FaRoC_Reader():
    def __init__(
        self,
        robot_ip :str = '172.22.5.81',         # ip address of fanuc controller
        port :int = 24087, # or 18735                      # the port of KAREL program "FAROC_SERVER"
    ...
    def handle_response(self, resp: bytes, d_type: str = None) -> (int, str, list):
        if self.debug_mode:
            print(f'resp|{len(resp)}[{resp}]')

        len_str = resp[0]  # first byte is the length of the received string
        format_ = f'{self.byte_order}{len_str}s'
        if self.debug_mode:
            print(f'format String: [{format_}]')
        ...
        code = int(code)
        # after the 1st string comes the interesting data
        data_byte = resp[1+len_str:]

        self.data_byte = data_byte

        if code == ERROR_CODE:
            # in this case KAREL sends only one INTEGER (4 bytes): "status"
            status = struct.unpack(f'{self.byte_order}l', data_byte[:4])[0]
            print(f'msg: {msg}')
            print(f"Fanuc Error status: '{status}'")
            if status > 0:
                self.error_detail(status)
            code = status  # code contains NOW the status code
        ...
        if d_type and code == SUCCESS_CODE:
            ...
            if d_type == 'S':  # the interesting data are STRINGS
                while len(data_byte) > 0:
                    ...
                    st = data_byte[1:1+len_str].decode('utf-8')
                    data.append(st)
                    data_byte = data_byte[1+len_str:]
            else:
                nr_data = int(len(data_byte) / 4)  # our server sends ONLY 4-byte DATA
                if nr_data > 0:
                    c_type = DTYPE_2_CTYPE[d_type]
                    # Use '<' for little-endian byte order
                    format_ = f'>{nr_data}{c_type}'
                    if self.debug_mode:
                        print(f'format non-String: [{format_}]')
...
###############################################################################
    ...
    def get_reg_as_integer(self, reg_nr: int) -> (int, str, int):
        """ Get the value of the DATA register number 'reg_nr' as an integer. """
        assert reg_nr > 0, f'Error: reg_nr must be positive, got: {reg_nr}.'
        cmd = f'get_reg:{reg_nr:03d}' 
        code, msg, data = self.send_cmd(cmd, d_type='I')  # Retrieve as integer
        print(f'Received data: {data}')  # Add this line for debugging
        if code == 0 and data:  # Check if data is not None
            print(f'First element of data: {data[0]}')  # Add this line for debugging
            return code, msg, data[0]  # Return the first element of the data list
        else:
            return code, msg, None
